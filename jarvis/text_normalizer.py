...
_TOPIC_SHIFT_MARKERS_TUPLE = tuple(TOPIC_SHIFT_MARKERS)
...
def normalize_text(
    ...
    normalized_lines = []
    for line in lines:
        # Collapse multiple spaces/tabs to single space (use split/join)
...
def starts_new_topic(text: str) -> bool:
    ...
    if not text:
        return False
    normalized = text.lower().strip()
    return normalized.startswith(_TOPIC_SHIFT_MARKERS_TUPLE)
