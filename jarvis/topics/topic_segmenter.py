...
def segment_conversation(
    ...
    # Pre-calculate all entity anchors in one batch (minimizes N+1 SpaCy overhead)
    all_texts = [m.text or "" for m in messages]
    all_message_anchors = anchor_tracker.get_anchors_batch(all_texts)

    segments: list[TopicSegment] = []
    current_chunk: list[Message] = [messages[0]]
    current_segment_anchors: set[str] = all_message_anchors[0]
