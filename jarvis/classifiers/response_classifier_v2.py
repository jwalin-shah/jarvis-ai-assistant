...
class BatchResponseClassifier(EmbedderMixin, CentroidMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            enable_ensemble: Enable ensemble voting across methods.
    ...
    def classify(
        ...
        # Need embedding for centroid/custom if no structural match
        ...
        if structural_type is not None:
            final_label = structural_type
            final_conf = self._structural_confidence
            method = "structural"
        elif centroid_type is not None:
            final_label = centroid_type
            final_conf = centroid_conf
            method = "centroid"
        else:
        ...
        if custom_result and custom_result[1] > final_conf:
            # Custom class logic would go here
            pass
        ...
        result = ClassificationResultV2(
            ...
            centroid_confidence=centroid_conf if centroid_type else None,
    ...
    def classify_batch(
        ...
        if needs_embedding_texts:
            ...
            for idx, orig_idx in enumerate(needs_embedding_indices):
                ...
                method = "centroid" if centroid_type is not None else "fallback"
                label = centroid_type if centroid_type is not None else ResponseTypeV2.ANSWER
                conf = centroid_conf if centroid_type is not None else 0.5
...
_use_v2_classifier: bool = True


def get_batch_response_classifier() -> BatchResponseClassifier:
    """Get the singleton BatchResponseClassifier instance."""
    return _factory.get()
...
def set_ab_test_config(config: ABTestConfig | None) -> None:
    """Set the A/B test configuration."""
    global _ab_test_config
    _ab_test_config = config


def set_use_v2_classifier(enabled: bool) -> None:
    """Enable or disable the V2 classifier."""
