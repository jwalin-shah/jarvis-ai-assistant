...
Usage:
    from jarvis.classifiers.trigger_classifier import classify_trigger, TriggerType
...
class HybridTriggerClassifier(EmbedderMixin, CentroidMixin):
    """Classifier for incoming messages using structural patterns + Centroids.

    Strategy:
    1. Check structural patterns (fast, high precision)
    2. If no match, try Centroid verification (semantic check)
    ...
    def __init__(
        ...
        """
        # Set model path (used by CentroidMixin)
    ...
    def _match_centroid(self, text: str) -> tuple[TriggerType | None, float]:
        ...
        Returns:
            Tuple of (trigger_type, confidence) or (None, 0.0).
        """
        if not self.centroids_available:
            return None, 0.0
        ...
        try:
            ...
            # Find nearest centroid using CentroidMixin
            label, similarity = self._find_nearest_centroid(embedding)
            ...
            try:
                ...
                # Use verify threshold for centroids
                if similarity < self.CENTROID_VERIFY_THRESHOLD:
                    return None, similarity
                return trigger_type, similarity
            except ValueError:
                return None, 0.0
    ...
    def classify(self, text: str) -> TriggerClassification:
        ...
        # Step 2: Try Centroid classification
        centroid_type, centroid_conf = self._match_centroid(normalized)
        if centroid_type is not None:
            # If structural had a partial match, check if they agree
            if struct_type is not None and struct_type == centroid_type:
                # They agree - boost confidence
                return TriggerClassification(
                    trigger_type=struct_type,
                    confidence=max(struct_conf, centroid_conf),
                    method="structural+centroid",
                    valid_response_types=TRIGGER_TO_RESPONSE_TYPES[struct_type],
                )

            return TriggerClassification(
                trigger_type=centroid_type,
                confidence=centroid_conf,
                method="centroid",
                valid_response_types=TRIGGER_TO_RESPONSE_TYPES[centroid_type],
            )
...
def classify_trigger(text: str) -> TriggerClassification:
    ...
    Returns:
        TriggerClassification with type, confidence, and valid response types.
    """
    return get_trigger_classifier().classify(text)
