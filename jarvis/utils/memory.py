...
def get_gpu_load_percent() -> float | None:
    ...
    try:
        ...
        subprocess.check_output(["/usr/sbin/sysctl", "-n", "machdep.cpu.brand_string"], text=True)  # nosec B603 B607
...
def get_macos_memory_pressure() -> MacOSMemoryPressure | None:
    ...
    try:
        # Parse vm_stat
        vm_stat = subprocess.check_output(["/usr/bin/vm_stat"], text=True)  # nosec B603 B607
        stats = {}
        ...
        # Get sysctl metrics
        sysctl = subprocess.check_output(["/usr/sbin/sysctl", "vm.memory_pressure"], text=True).strip()  # nosec B603 B607
