...
"""
            return self._run_applescript(
                applescript, f"attachment to {chat_id or recipient}", [actual_chat_id, abs_path]
            )
        else:
            if not recipient:
...
"""
            return self._run_applescript(
                applescript, f"attachment to {chat_id or recipient}", [recipient, abs_path]
            )
...
"""
        return self._run_applescript(applescript, f"individual: {recipient}", [text, recipient])
    ...
    tell application "Messages"
        ...
        on error
            ...
            repeat with c in chats
                if id of c is targetChatId then
                    send messageText to c
                    return
                end if
            end repeat
            error "Chat not found: " & targetChatId
        end try
    end tell
end run
"""
