...
on run argv
    ...
    tell application "Messages"
        set targetChat to chat id theChatId
    ...
    tell application "Messages"
        ...
        on error
            ...
            repeat with c in chats
                if id of c is theChatId then
                    send theText to c
                    return
                end if
            end repeat
            error "Chat not found: " & theChatId
        end try
    end tell
end run
"""
        return self._run_applescript(
            applescript, f"group: {chat_id}", [text, actual_chat_id]
        )
    ...
    def _run_applescript(
        ...
        try:
            ...
            cmd = ["/usr/bin/osascript", "-e", script] + (args or [])
