...
from evals.eval_pipeline import EVAL_DATASET_PATH, check_anti_ai, load_eval_dataset  # noqa
from evals.judge_config import JUDGE_MODEL, get_judge_client  # noqa
...
def run_variant(
    ...
    for batch_idx in tqdm(range(num_batches), desc=f"Processing {name}"):
        ...
        for i, (ex, (reply, latency), anti_ai, (score, reasoning)) in enumerate(
            ...
            print(
                f"[{start_idx + i + 1:2d}] [{ex.category:12s}] {status} | {score:.0f}/10 | {reply[:50]}"  # noqa
            )
...
def main() -> int:
    ...
    for s in all_summaries:
        print(f"\n{s['name'].upper()}")
        print(
            f"  Config: context_depth={s['config']['context_depth']}, use_rag={s['config']['use_rag']}"  # noqa
        )
    ...
    print(
        f"Config: context_depth={winner['config']['context_depth']}, use_rag={winner['config']['use_rag']}"  # noqa
    )
