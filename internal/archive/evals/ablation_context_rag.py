...
def run_variant(
    ...
    for batch_idx in tqdm(range(num_batches), desc=f"Processing {name}"):
        ...
        for i, (ex, (reply, latency), anti_ai, (score, reasoning)) in enumerate(
            ...
            print(
                f"[{start_idx + i + 1:2d}] [{ex.category:12s}] {status} | {score:.0f}/10 | {reply[:50]}"  # noqa: E501  # noqa: E501
            )
...
def main() -> int:
    ...
    for s in all_summaries:
        print(f"\n{s['name'].upper()}")
        print(
            f"  Config: context_depth={s['config']['context_depth']}, use_rag={s['config']['use_rag']}"  # noqa: E501  # noqa: E501
        )
    ...
    print(
        f"Config: context_depth={winner['config']['context_depth']}, use_rag={winner['config']['use_rag']}"  # noqa: E501  # noqa: E501
    )
