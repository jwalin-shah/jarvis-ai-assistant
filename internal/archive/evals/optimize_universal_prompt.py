...
from evals.dspy_client import DSPYMLXClient  # noqa
from evals.dspy_reply import (  # noqa
    TRAIN_EXAMPLES,
    judge_metric,
)
...
class UniversalReplySignature(dspy.Signature):
    ...
    reply: str = dspy.OutputField(
        desc="Brief, natural reply matching the user's style. No AI filler."
    )
...
def prepare_trainset(depth: int):
    ...
    for ex in TRAIN_EXAMPLES:
        ...
        new_ex = dspy.Example(
            context=ctx_str, last_message=ex.last_message, tone=ex.tone, user_style=ex.user_style
...
def main():
    ...
    parser.add_argument(
        "--depths", type=str, default="3,5,10", help="Comma-separated context depths to test"
    )
    ...
    for depth in depths:
        ...
        res = {"depth": depth, "avg_score": avg_score, "save_path": str(save_path)}
        results.append(res)
