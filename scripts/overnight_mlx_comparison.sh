#!/usr/bin/env bash
# Overnight MLX Model Comparison (No Ollama Required!)
#
# Uses JARVIS's existing MLX infrastructure to test models:
# 1. Template mining (all-mpnet-base-v2)
# 2. LLM comparison (existing + new models via MLX)
#
# Models tested:
# - Qwen2.5-0.5B-Instruct-4bit (already in registry)
# - Qwen2.5-1.5B-Instruct-4bit (already in registry - current)
# - Qwen2.5-3B-Instruct-4bit (already in registry)
# - Gemma 3 4B (NEW - add to registry)
# - Qwen3 4B (NEW - add to registry)
#
# Runtime: 6-8 hours
# Memory: Safe for 8GB RAM (MLX is memory efficient!)

set -uo pipefail  # Removed -e so script continues on errors

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
RESULTS_DIR="results/overnight_mlx_${TIMESTAMP}"
mkdir -p "${RESULTS_DIR}"
...
log "════════════════════════════════════════════════════════════"
log "JARVIS Overnight: MLX Model Comparison (No Ollama!)"
log "════════════════════════════════════════════════════════════"
log "Started: $(date)"
...
log "  Part 2: 6 LLM Models:"
log "    • Qwen2.5-1.5B (current baseline)"
log "    • SmolLM3-3B (beats Qwen2.5-3B!)"
log "    • Ministral-3B (beats Gemma 3 4B!)"
log "    • Phi-3-Mini (fastest - 28 tok/s)"
log "    • Gemma 3 4B (instruction following)"
log "    • Qwen3 4B (74% MMLU - highest)"
log ""
...
log "┌────────────────────────────────────────────────────────────┐"
log "│ Experiment 2: MLX Model Quality Tests                     │"
log "│ Testing 5 models on iMessage reply scenarios              │"
log "└────────────────────────────────────────────────────────────┘"
log ""
...
#!/usr/bin/env python3
"""Test MLX models for response generation quality."""
...
# Models to test - 6 BEST models for 8GB RAM
MODELS = {
    "qwen-1.5b": {
    ...
    "smollm3-3b": {
        ...
        "model_path": "mlx-community/SmolLM3-3B-Instruct-4bit",
        "params": "3B",
        "memory": "~2GB",
        "notes": "Beats Qwen2.5-3B & Llama-3.2-3B! Dual reasoning mode"
    },
    "ministral-3b": {
        "name": "Ministral-3B-Instruct",
        "model_path": "mlx-community/Ministral-3B-Instruct-4bit",
        "params": "3.4B",
        "memory": "~2.3GB",
        "notes": "BEATS Gemma 3 4B! 256K context, vision support"
    },
    "phi3-mini": {
    ...
    "qwen3-4b": {
        ...
        "params": "4B",
        "memory": "~2.75GB",
        "notes": "HIGHEST MMLU (74%), best reasoning"
    }
}
...
# JARVIS Overnight Results: MLX Model Comparison

**Date:** $(date '+%Y-%m-%d')
**Duration:** ${TOTAL_HOURS} hours
**Infrastructure:** MLX (Apple Silicon optimized) - No Ollama needed!

---

## Models Tested

### Complete Lineup (6 models)
1. **Qwen2.5-1.5B** (1.5B params, ~1.5GB) - Current baseline
2. **SmolLM3-3B** (3B params, ~2GB) - Beats Qwen2.5-3B & Llama-3.2-3B
3. **Ministral-3B** (3.4B params, ~2.3GB) - Beats Gemma 3 4B!
4. **Phi-3-Mini** (3.8B params, ~2.5GB) - Fastest (28 tok/s)
5. **Gemma 3 4B** (4B params, ~2.75GB) - Best instruction following
6. **Qwen3 4B** (4B params, ~2.75GB) - Highest MMLU (74%)
...
- `test_mlx_models.py` - Test script
...
if [[ -f "${RESULTS_DIR}/mlx_comparison_results.json" ]]; then
    ...
    if [[ "${MODELS_SUCCESS}" != "${MODELS_TESTED}" ]]; then
        log ""
        log "  Failed models:"
        jq -r '.models | to_entries[] | select(.value.load_error or .value.error) | "    • \(.value.info.name): \(.value.load_error // .value.error)"' \
