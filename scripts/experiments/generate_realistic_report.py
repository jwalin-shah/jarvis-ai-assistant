...
- **Verdict:** {
        "✅ Good coverage"
        if template["hit_rate"] > 0.3
        else "⚠️ Low coverage - needs more templates"
    }
...
"""
    ...
    for model_result in llm:
        ...
        report += (
            f"| {model_result['model']} | {stats['avg_latency_ms']:.0f}ms | "
            f"{stats['avg_tokens']:.0f} | {stats['avg_length_chars']:.0f} chars | "
            f"{stats['successes']}/{stats['total_tests']} |\n"
        )
...
"""
    ...
    if fastest:
        report += (
            ...
            f"({fastest['stats']['avg_latency_ms']:.0f}ms avg)\n"
        )
    ...
    if most_concise:
        report += (
            f"- 📝 **Most Concise:** {most_concise['model']} "
            f"({most_concise['stats']['avg_length_chars']:.0f} chars avg)\n"
        )

    report += "\n### Quality Metrics\n"
    report += f"- **Brevity:** {quality['brevity_score']:.1%} of responses are brief (<100 chars)\n"
    report += (
        f"- **Variety:** {quality['variety_score']:.2f} unique responses per context "
...
"""
    ...
    if template["hit_rate"] > 0:
        ...
        report += (
            f"- Template hits: {template['hit_rate']:.1%} × "
            f"{template['avg_latency_ms']:.0f}ms = {template_time:.0f}ms\n"
        )
        llm_latency = fastest["stats"]["avg_latency_ms"] if fastest else 500
        report += (
            f"- LLM fallback: {1 - template['hit_rate']:.1%} × "
            f"{llm_latency:.0f}ms = {llm_time:.0f}ms\n"
        )
