...
def generate_reply_with_context(
    ...
    if is_group:
        prompt = f"""You are drafting a brief iMessage reply in a group chat. \
{tone_instruction} Keep it to 1 sentence max.
...
Your reply:"""
        ...
        prompt = f"""You are drafting a brief iMessage reply. \
{tone_instruction} Keep it to 1 sentence max.
...
def test_llm_generation_enhanced(
    ...
    for thread in threads[:10]:
        _ = thread["display_name"] or "Unknown"  # Available if needed for logging
...
def main():
    ...
    tmpl_quality = quality_metrics["template_quality"]
    print(f"  Template appropriateness: {tmpl_quality['avg_appropriateness']:.2f}")
