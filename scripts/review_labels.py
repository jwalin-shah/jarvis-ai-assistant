...
def main():
    ...
    parser.add_argument(
        "--type", choices=["trigger", "response"], required=True, help="Which dataset to review"
    )
    parser.add_argument(
        "--limit", type=int, default=50, help="Max examples to review (default: 50)"
    )
    parser.add_argument(
        "--pattern",
        type=str,
    ...
    parser.add_argument(
        "--stats-only", action="store_true", help="Only show statistics, don't review"
    )
    parser.add_argument(
        "--output",
        type=Path,
