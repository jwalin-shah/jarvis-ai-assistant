...
TEST_CASES: list[TestCase] = [
    ...
    },
    {
        ...
        "thread": ["Alex: See you later!", "You: Sounds good! Can't wait."],
...
def run_bakeoff() -> None:
    ...
    for conv in conversations:
        ...
        [
            f"{'You' if m.is_from_me else (m.sender_name or 'Them')}: {m.text}"
        ...
        classification = service._build_classification_result(
            incoming, [m.text for m in reversed(messages) if m.text], mobilization
        )
        t_class_end = time.perf_counter()
        print(f"   [DEBUG] Classification took {(t_class_end - t_class_start) * 1000:.1f}ms")

        print(
            f"   Classification: {classification.metadata.get('category_name')} (conf={classification.confidence:.2f})"
        )

        print("   [DEBUG] Calling generate_reply (this often hangs)...")
        start_a = time.perf_counter()
        response_a = service.generate_reply(
            message_context, classification=classification, contact=contact
        )
        ...
        forced_classification = ClassificationResult(
            ...
            metadata={"category_name": "statement", "category_method": "bakeoff_forced"},
        )

        response_b = service.generate_reply(
            message_context, classification=forced_classification, contact=contact
        )
        ...
        response_f = service.generate_reply(
            message_context, classification=direct_classification, contact=contact
        )
