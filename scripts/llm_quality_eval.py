...
Usage:
    python scripts/llm_quality_eval.py                    # Run with defaults
    python scripts/llm_quality_eval.py --samples 100      # Custom sample size
    python scripts/llm_quality_eval.py --test-ratio 0.3   # 30% test set
    python scripts/llm_quality_eval.py --verbose          # Detailed logging
"""
...
class HoldoutEvaluator:
    ...
    def split_data(
        self, pairs: list[Any], seed: int = RANDOM_SEED
    ...
    def compare_single(
        ...
        template_coherence = (
            score_response_coherence(trigger, template_response)
            if template_response
            else 0.0
        )
        llm_coherence = (
            score_response_coherence(trigger, llm_response) if llm_response else None
        )
    ...
    def run_evaluation(
        ...
        summary = EvaluationSummary(
            ...
            avg_template_similarity=(
                sum(similarities) / len(similarities) if similarities else 0
            ),
            avg_template_coherence=(
                sum(template_coherences) / len(template_coherences)
                if template_coherences
                else 0
            ),
            avg_llm_coherence=(
                sum(llm_coherences) / len(llm_coherences) if llm_coherences else 0
            ),
            avg_comparison_time_ms=(
                sum(r.comparison_time_ms for r in results) / len(results)
                if results
                else 0
            ),
