#!/usr/bin/env python3
"""Train and evaluate trigger classifier with different configurations.

Tries multiple sampling strategies and hyperparameters to find the best setup.

Usage:
    uv run python -m scripts.train_trigger_classifier
...
def main():
    ...
    parser.add_argument(
        ...
        help="Input JSONL file with labeled triggers",
    )
    parser.add_argument(
        "--save-best",
        action="store_true",
        help="Save the best model to ~/.jarvis/trigger_classifier_model/",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/trigger_classifier_experiments.json"),
