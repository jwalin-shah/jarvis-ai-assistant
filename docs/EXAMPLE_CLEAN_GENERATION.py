...
if TYPE_CHECKING:
    from jarvis.pipelines.classify import Category, Classification
...
def get_template_reply(category: Category) -> Reply:
    ...
    if category == Category.ACKNOWLEDGE:
        return Reply(
            text=random.choice(ACKNOWLEDGE_TEMPLATES), confidence=1.0, method="template_ack"
        )
    elif category == Category.CLOSING:
        return Reply(
            text=random.choice(CLOSING_TEMPLATES), confidence=1.0, method="template_closing"
        )
    else:
...
class ContextAssembler:
    ...
    def assemble(
        self, message: str, chat_id: str | None, classification: Classification
    ) -> Context:
        """
...
class GenerationPipeline:
    ...
    def generate(
        self, message: str, classification: Classification, chat_id: str | None = None
    ) -> Reply:
        """
        ...
        try:
            ...
            confidence = self._calculate_confidence(
                classification=classification, context=context, response=response_text
            )
    ...
    def _calculate_confidence(
        self, classification: Classification, context: Context, response: str
    ) -> float:
        """
...
def generate_reply(
    message: str, classification: Classification, chat_id: str | None = None
) -> Reply:
    """
    Generate a reply to a message.
...
if __name__ == "__main__":
    from jarvis.pipelines.classify import Category, classify_message
    ...
    print(
        f"Classification: {classification.category.value}, urgency={classification.urgency.value}"
    )
