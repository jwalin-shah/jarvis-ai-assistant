...
**Script:** `scripts/experiment_clustering.py`
...
uv run python -m scripts.experiment_clustering --mlx --k 5 10 15

# HDBSCAN (auto-detects number of clusters)
uv run python -m scripts.experiment_clustering --hdbscan --min-cluster-size 50

# Test all encoder models
uv run python -m scripts.experiment_clustering --models bge-small gte-tiny minilm-l6 bge-micro

# Full run with cluster samples printed
uv run python -m scripts.experiment_clustering --mlx --k 5 10 15 20 --print-samples
```
...
| Script | Purpose |
|--------|---------|
| `scripts/experiment_clustering.py` | Unsupervised clustering on all messages |
| `scripts/experiment_all_encoders.py` | Compare 4 encoder models on zero-shot |
| `scripts/experiment_embedding_zeroshot_v2.py` | Test different category schemes |
| `scripts/experiment_llm_labeling.py` | LLM-based labeling (deprecated approach) |
