...
LOCATION_PATTERNS = [
    ...
    # Past
    (re.compile(r"\b(?:grew\s+up\s+in|lived\s+in|from)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b", re.I),
...
def get_understanding_pipeline() -> UnderstandingPipeline:
    ...
    if _pipeline is None:
        _pipeline = UnderstandingPipeline(
...
if __name__ == "__main__":
    ...
    for msg in messages:
        ...
        for fact in knowledge.facts:
            print(f"  Fact: {fact.subject} --{fact.predicate}--> {fact.object or fact.value} (conf={fact.confidence})")
