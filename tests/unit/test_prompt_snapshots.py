...
def test_reply_prompt_snapshot(snapshot):
    ...
    prompt = build_reply_prompt(
        context=context, last_message=last_message, instruction="be nice", tone="casual"
    )
...
def test_threaded_reply_prompt_snapshot(snapshot):
    ...
    prompt = build_threaded_reply_prompt(
        thread_context=thread_context, config=MockConfig(), instruction="confirm 5pm", tone="casual"
    )
