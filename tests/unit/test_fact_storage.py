...
class TestSaveCandidateFacts:
    """Test save_candidate_facts conversion from FactCandidate to Fact."""

    def test_converts_and_saves_known_fact_type(self, patched_get_db) -> None:
        """Known fact_type maps to correct category/predicate."""
        from jarvis.contacts.candidate_extractor import FactCandidate
        from jarvis.contacts.fact_storage import get_facts_for_contact, save_candidate_facts
        ...
        candidate = FactCandidate(
            ...
            start_char=10,
            end_char=16,
            source_text="I live in Austin now",
        )

        inserted = save_candidate_facts([candidate], "contact-001")
        assert inserted == 1
        ...
        assert stored[0].predicate == "lives_in"
        assert stored[0].subject == "Austin"
        assert stored[0].confidence == pytest.approx(0.85, abs=0.01)

    def test_unknown_fact_type_skipped(self, patched_get_db) -> None:
        """Unknown fact_type is silently skipped."""
        from jarvis.contacts.candidate_extractor import FactCandidate
    ...
    def test_zero_gliner_score_defaults_to_half(self, patched_get_db) -> None:
        """If gliner_score <= 0, confidence defaults to 0.5."""
        from jarvis.contacts.candidate_extractor import FactCandidate
        from jarvis.contacts.fact_storage import get_facts_for_contact, save_candidate_facts
        ...
        save_candidate_facts([candidate], "contact-001")

        stored = get_facts_for_contact("contact-001")
        assert len(stored) == 1
        assert stored[0].confidence == pytest.approx(0.5, abs=0.01)

    def test_empty_candidates_returns_zero(self, patched_get_db) -> None:
        """Empty candidate list returns 0."""
        from jarvis.contacts.fact_storage import save_candidate_facts

        inserted = save_candidate_facts([], "contact-001")
        assert inserted == 0

    def test_non_factcandidate_objects_skipped(self, patched_get_db) -> None:
        """Non-FactCandidate objects in list are skipped gracefully."""
        from jarvis.contacts.fact_storage import save_candidate_facts

        inserted = save_candidate_facts(
            ["not-a-candidate", 42],
            "contact-001",  # type: ignore[list-item]
        )
        assert inserted == 0

    def test_all_type_mappings(self, patched_get_db) -> None:
        """Verify all 19 fact_type -> (category, predicate) mappings work."""
        from jarvis.contacts.candidate_extractor import FactCandidate
        from jarvis.contacts.fact_storage import get_facts_for_contact, save_candidate_facts
        ...
        candidates = []
        for i, (fact_type, _) in enumerate(type_map.items()):
            candidates.append(
                FactCandidate(
                    message_id=i,
                    span_text=f"Entity{i}",
                    span_label="test_label",
                    gliner_score=0.8,
                    fact_type=fact_type,
                    start_char=0,
                    end_char=7,
                    source_text=f"Source text for entity {i}",
                )
            )

        contact_id = "contact-mapping-test"
        inserted = save_candidate_facts(candidates, contact_id)
        assert inserted == len(type_map)

        stored = get_facts_for_contact(contact_id)
        stored_tuples = {(f.category, f.predicate) for f in stored}

        for expected in type_map.values():
            assert expected in stored_tuples, f"Missing mapping: {expected}"
