...
def test_generation_pipeline_mocked():
    ...
    mock_loader.generate_sync.return_value = GenerationResult(
        text="Mocked response", tokens_generated=5, generation_time_ms=10.0
    )
    ...
    request = GenerationRequest(
        prompt="Hello", context_documents=[], few_shot_examples=[], max_tokens=50
    )
...
def test_real_model_loading_and_generation():
    ...
    try:
        ...
        result = loader.generate_sync(prompt=prompt, max_tokens=10, temperature=0.1)
