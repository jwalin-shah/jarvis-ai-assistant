    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
          Bulk Tag {selectedConversations.length} Conversation{selectedConversations.length !== 1 ? "s" : ""}
        </h2>
        <button
          type="button"
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
          <p class="text-sm text-blue-700 dark:text-blue-300">
            {#if canApply}
              This will {action} <strong>{selectedTagIds.length}</strong> tag{selectedTagIds.length !== 1 ? "s" : ""}
              {action === "add" ? "to" : "from"} <strong>{selectedConversations.length}</strong> conversation{selectedConversations.length !== 1 ? "s" : ""}.
