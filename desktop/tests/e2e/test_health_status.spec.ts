...
test.describe('Health Status Panel', () => {
  ...
  test('displays JARVIS process metrics', async ({ mockedPage: page }) => {
    ...
    await expect(processCard).toContainText(`${mockHealthResponse.jarvis_rss_mb.toFixed(0)} MB`);
  ...
  test('navigates to health from sidebar', async ({ mockedPage: page }) => {
    ...
    await expect(page.locator('.nav-item[title="Health Status"]')).toHaveClass(/active/);
