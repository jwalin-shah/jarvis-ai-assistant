...
import { test, expect, waitForAppLoad, selectConversation } from './fixtures';
...
test.describe('AI Draft Panel', () => {
  ...
  test('draft panel can be opened', async ({ mockedPage: page }) => {
    ...
    if (count > 0) {
      ...
      // Panel should appear
      await expect(page.locator('.panel, [role="dialog"], .ai-draft-panel')).toBeVisible();
    } else {
  ...
  test('draft panel has header with title', async ({ mockedPage: page }) => {
    const draftButton = page.locator('button:has-text("Draft"), button:has-text("AI"), .draft-btn');
    const count = await draftButton.count();
    ...
    if (count > 0) {
      ...
      await expect(page.locator('.panel-header, .modal-header')).toContainText('AI Draft');
    } else {
      test.skip(true, 'Draft button not found in UI');
    }
  });
  ...
  test('draft panel has instruction input', async ({ mockedPage: page }) => {
    ...
    if (count > 0) {
      ...
      await expect(instructionInput).toHaveAttribute('placeholder', /what do you want|say/i);
    } else {
      test.skip(true, 'Draft button not found in UI');
    }
  });
  ...
  test('clicking generate shows loading state', async ({ mockedPage: page }) => {
    const draftButton = page.locator('button:has-text("Draft"), button:has-text("AI"), .draft-btn');
    const count = await draftButton.count();
  ...
  test('generates and displays suggestions', async ({ mockedPage: page }) => {
    ...
    if (count > 0) {
      ...
      await expect(suggestions).toHaveCount(mockDraftReplyResponse.suggestions.length);
    } else {
      test.skip(true, 'Draft button not found in UI');
    }
  });

  test('suggestions display text and confidence', async ({ mockedPage: page }) => {
    const draftButton = page.locator('button:has-text("Draft"), button:has-text("AI"), .draft-btn');
    const count = await draftButton.count();
    ...
    if (count > 0) {
      ...
      await expect(firstSuggestion).toContainText(mockDraftReplyResponse.suggestions[0].text);

      // Confidence badge
      const confidence = Math.round(mockDraftReplyResponse.suggestions[0].confidence * 100);
  ...
  test('can select a suggestion', async ({ mockedPage: page }) => {
    ...
    if (count > 0) {
      ...
      await expect(page.locator('.suggestion-item.selected, .suggestion.selected')).toBeVisible();
    } else {
      test.skip(true, 'Draft button not found in UI');
    }
  });

  test('use selected button is enabled when suggestion selected', async ({ mockedPage: page }) => {
    const draftButton = page.locator('button:has-text("Draft"), button:has-text("AI"), .draft-btn');
    const count = await draftButton.count();
  ...
  test('regenerate button generates new suggestions', async ({ mockedPage: page }) => {
    const draftButton = page.locator('button:has-text("Draft"), button:has-text("AI"), .draft-btn');
    const count = await draftButton.count();
    ...
    if (count > 0) {
      ...
      await page.waitForSelector('.suggestion-item, .suggestion');
      await expect(page.locator('.suggestion-item, .suggestion')).toHaveCount(
