...
test.describe("Error States and Recovery", () => {
  test.describe("API Error Handling", () => {
    test("shows error banner when health check fails", async ({
      ...
      const hasError =
        ...
        (await disconnectedStatus.count()) > 0;
    ...
    test("shows specific error message for API failures", async ({
      page,
    }) => {
  ...
  test.describe("Network Disconnection", () => {
    test("shows disconnected status when offline", async ({
      mockedPage: page,
    }) => {
      await page.goto("/");
    ...
    test("app remains functional with cached data when offline", async ({
      mockedPage: page,
    }) => {
      await page.goto("/");
      await waitForAppLoad(page);
    ...
    test("retries connection after going online", async ({
      mockedPage: page,
    }) => {
      await page.goto("/");
      await waitForAppLoad(page);
  ...
  test.describe("Partial Failure Handling", () => {
    ...
    test("shows degraded mode indicator when partial failure", async ({
      page,
    }) => {
      await page.route("http://localhost:8742/health", async (route) => {
        await route.fulfill({
  ...
  test.describe("Error Recovery", () => {
    test("clears error state when data loads successfully", async ({
      page,
    }) => {
      let shouldFail = true;
    ...
    test("state is preserved during transient errors", async ({
      mockedPage: page,
    }) => {
      await page.goto("/");
      await waitForAppLoad(page);
