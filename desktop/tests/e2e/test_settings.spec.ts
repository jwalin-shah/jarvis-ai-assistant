...
test.describe('Settings Page', () => {
  ...
  test('displays model selection section', async ({ mockedPage: page }) => {
    ...
    const modelSection = page
      .locator('.section')
      .filter({ has: page.locator('h2', { hasText: 'Model' }) });
  ...
  test('shows recommended badge on recommended model', async ({ mockedPage: page }) => {
  ...
  test('shows model details (size, quality, RAM)', async ({ mockedPage: page }) => {
    // First model card
    ...
    await expect(firstModel).toContainText(`Requires ${mockModels[0].ram_requirement_gb} GB RAM`);
  });

  test('displays generation settings section', async ({ mockedPage: page }) => {
    const generationSection = page
      .locator('.section')
      .filter({ has: page.locator('h2', { hasText: 'Generation' }) });
  ...
  test('shows temperature slider', async ({ mockedPage: page }) => {
    ...
    await expect(page.locator('label', { hasText: 'Temperature' }).locator('.value')).toHaveText(
      mockSettingsResponse.generation.temperature.toFixed(1)
    );
  });

  test('shows max tokens sliders', async ({ mockedPage: page }) => {
    // Reply max tokens
    await expect(page.locator('label', { hasText: 'Reply Max Tokens' })).toBeVisible();
    await expect(page.locator('#maxTokensReply')).toBeVisible();

    // Summary max tokens
    await expect(page.locator('label', { hasText: 'Summary Max Tokens' })).toBeVisible();
    await expect(page.locator('#maxTokensSummary')).toBeVisible();
  });

  test('displays suggestions settings section', async ({ mockedPage: page }) => {
    const suggestionsSection = page
      .locator('.section')
      .filter({ has: page.locator('h2', { hasText: 'Suggestions' }) });
  ...
  test('shows conditional settings when auto-suggest is on', async ({ mockedPage: page }) => {
    // When auto-suggest is on, should show suggestion count slider
    await expect(page.locator('label', { hasText: 'Suggestions Count' })).toBeVisible();
    await expect(page.locator('label', { hasText: 'Context Messages (Replies)' })).toBeVisible();
    await expect(page.locator('label', { hasText: 'Context Messages (Summaries)' })).toBeVisible();
  });

  test('hides conditional settings when auto-suggest is off', async ({ mockedPage: page }) => {
    // Find the Suggestions section first
    const suggestionsSection = page
      .locator('.section')
      .filter({ has: page.locator('h2', { hasText: 'Suggestions' }) });
    ...
    await expect(page.locator('label', { hasText: 'Suggestions Count' })).not.toBeVisible();
  });

  test('displays system information section', async ({ mockedPage: page }) => {
    const systemSection = page
      .locator('.section')
      .filter({ has: page.locator('h2', { hasText: 'System' }) });
  ...
  test('shows system info values', async ({ mockedPage: page }) => {
    // System RAM
    await expect(page.locator('.info-row', { hasText: 'System RAM' })).toBeVisible();
    await expect(
      page.locator('.info-row', { hasText: 'System RAM' }).locator('.info-value')
    ).toContainText('GB');

    // Current Memory Usage
    await expect(page.locator('.info-row', { hasText: 'Current Memory Usage' })).toBeVisible();

    // Model Status
    await expect(page.locator('.info-row', { hasText: 'Model Status' })).toBeVisible();

    // iMessage Access
    await expect(page.locator('.info-row', { hasText: 'iMessage Access' })).toBeVisible();
  });
  ...
  test('save button saves settings', async ({ mockedPage: page }) => {
    ...
    await expect(page.locator('.success-banner')).toContainText('Settings saved successfully');
  });
  ...
  test('shows download button for not-downloaded models', async ({ mockedPage: page }) => {
