...
test.describe('Accessibility Tests', () => {
  test.describe('ARIA Labels and Roles', () => {
    ...
    test('conversation list has proper list semantics', async ({ mockedPage: page }) => {
      ...
      const conversationList = page.locator('.conversation-list, [role="list"], ul.conversations');

      if ((await conversationList.count()) > 0) {
        // List items should have proper roles
        const items = conversationList.locator('.conversation, [role="listitem"], [role="option"]');
  ...
  test.describe('Keyboard Navigation', () => {
    ...
    test('can navigate sidebar with arrow keys', async ({ mockedPage: page }) => {
      ...
      await expect(
        page.locator(".nav-item:nth-child(2), .nav-item.focused, .nav-item[data-focused='true']")
    ...
    test('can navigate conversations with keyboard', async ({ mockedPage: page }) => {
      ...
      await expect(
        page
          .locator(
            ".conversation:nth-child(2), .conversation.focused, .conversation[aria-selected='true']"
          )
          .first()
      ).toBeFocused({ timeout: 1000 });
    ...
    test('search modal can be opened with keyboard shortcut', async ({ mockedPage: page }) => {
      await page.goto('/');
      await waitForAppLoad(page);
    ...
    test('search results can be navigated with arrow keys', async ({ mockedPage: page }) => {
      await page.goto('/');
      await waitForAppLoad(page);
      ...
      await page.waitForSelector(".result-item, .search-result, [role='option']", {
        timeout: 2000,
      });
      ...
      await page.waitForSelector(
        ".result-item.selected, [aria-selected='true'], .result-item.focused",
        { timeout: 1000 }
      );
    ...
    test('Space key activates checkboxes and toggles', async ({ mockedPage: page }) => {
      ...
      if ((await toggle.count()) > 0) {
        ...
        await page.waitForFunction(
          ...
          {
            el: ".toggle-btn, input[type='checkbox']",
            initial: { state: initialState, checked: initialChecked },
          },
          { timeout: 1000 }
        );
  ...
  test.describe('Focus Management', () => {
    ...
    test('focus moves to error messages', async ({ errorPage: page }) => {
      ...
      const errorElement = page.locator(".error, .error-banner, .error-message, [role='alert']");
