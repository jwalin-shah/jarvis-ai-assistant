...
from benchmarks.latency.timer import HighPrecisionTimer, force_model_unload, warmup_timer
...
def benchmark_v1_classifier(messages: list[str], batch_size: int = 64) -> BenchmarkResult:
    ...
    result.metadata = {
        "has_svm": classifier._svm is not None,
        "has_centroids": bool(classifier.centroids),
    }
...
def main() -> int:
    ...
    if args.compare:
        results = compare_classifiers(messages, args.batch_size)
    else:
        print(f"\nRunning {args.benchmark} benchmark...")
        results = benchmark_v2_classifier(messages, args.batch_size)
