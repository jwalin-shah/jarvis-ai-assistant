...
def load_cross_encoder_weights(model: BertForSequenceClassification, weights_path: Path) -> None:
    ...
    for hf_name, weight in bert_weights.items():
        ...
        if "encoder.layer." in name_inner:
            ...
            name_inner = name_inner.replace(".attention.output.dense", ".attention_output_dense")
    ...
    def predict(
        ...
        with self._get_gpu_lock():
            ...
            for batch_start in range(0, len(pairs), batch_size):
                ...
                input_ids = np.array(
                    [e.ids[:max_len] for e in batch_encodings],
                    dtype=np.int32,
                )
                attention_mask = np.array(
                    [e.attention_mask[:max_len] for e in batch_encodings],
                    dtype=np.int32,
                )
                token_type_ids = np.array(
                    [e.type_ids[:max_len] for e in batch_encodings],
                    dtype=np.int32,
                )
